...
import (
	...
	"github.com/commanderu/cdrwallet/version"
...
const (
	defaultCAFilename          = "cdr.cert"
	defaultConfigFilename      = "cdrwallet.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "cdrwallet.log"
	...
	defaultMaxFee                    cdrutil.Amount = 1e6
	defaultMinFee                    cdrutil.Amount = 1e5
...
var (
	cdrDefaultCAFile  = filepath.Join(cdrutil.AppDataDir("cdr", false), "rpc.cert")
	defaultAppDataDir  = cdrutil.AppDataDir("cdrwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of cdr RPC server to connect to"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with cdr"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	cdrUsername     string                  `long:"cdrusername" description:"Username for cdr authentication"`
	cdrPassword     string                  `long:"cdrpassword" default-mask:"-" description:"Password for cdr authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy JSON-RPC and cdr authentication (if cdrusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy JSON-RPC and cdr authentication (if cdrpassword is unset)"`
...
//
// The above results in cdrwallet functioning properly without any config
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local cdr cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local cdr and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					cdrCertExists, err := cfgutil.FileExists(
						cdrDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					...
					if cdrCertExists {
						cfg.CAFile.Value = cdrDefaultCAFile
					}
				}
			}
		}
	...
	// If the cdr username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for cdr and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for cdr and wallet.
	if cfg.cdrUsername == "" {
		cfg.cdrUsername = cfg.Username
	}
	if cfg.cdrPassword == "" {
		cfg.cdrPassword = cfg.Password
	}
