...
func testExternalAddresses(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i := 0; i < len(expectedExternalAddrs); i++ {
			...
			utilAddr, err := cdrutil.NewAddressPubKeyHash(pkHash,
...
func testImportPrivateKey(tc *testContext) bool {
	...
	if tc.create {
		for i, test := range tests {
			...
			wif, err := cdrutil.DecodeWIF(test.in)
			if err != nil {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			utilAddr, err := cdrutil.NewAddressPubKeyHash(
...
func testImportScript(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			// Use the Address API to retrieve each of the expected
			// new addresses and ensure they're accurate.
			utilAddr, err := cdrutil.NewAddressScriptHash(test.in,
				chainParams)
			if err != nil {
...
func testMarkUsed(tc *testContext) bool {
	...
	for i, test := range tests {
		...
		case addrPubKeyHash:
			...
			addr, err = cdrutil.NewAddressPubKeyHash(addrHash, chainParams, chainec.ECTypeSecp256k1)
		case addrScriptHash:
			testtype = "addrScriptHash"
			addr, err = cdrutil.NewAddressScriptHashFromHash(addrHash, chainParams)
		default:
			panic("unreachable")
		}
		if err != nil {
...
func testLookupAccount(tc *testContext) bool {
	...
	for i, addr := range expectedAddrs {
		addr, err := cdrutil.NewAddressPubKeyHash(addr.addressHash,
...
func testSync(tc *testContext) bool {
	tests := []struct {
		...
		{
			name: "Block 312",
			hash: newShaHash("00000000ecd989cc1b19caf707b686a1f0071374602fb7c5d5d559da96137c8ce"),
		},
		{
			name: "Block 313",
