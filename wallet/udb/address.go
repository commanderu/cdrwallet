...
type ManagedAddress interface {
	...
	// Address returns a cdrutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *cdrutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the cdrutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() cdrutil.Address {
	return a.address
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey chainec.PublicKey, compressed bool) (*managedAddress, error) {
	...
	var pubKeyHash []byte
	if compressed {
		pubKeyHash = cdrutil.Hash160(pubKey.SerializeCompressed())
	} else {
		pubKeyHash = cdrutil.Hash160(pubKey.SerializeUncompressed())
	}
	address, err := cdrutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams,
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *cdrutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() cdrutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := cdrutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
