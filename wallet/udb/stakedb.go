...
func deserializeSStxRecord(serializedSStxRecord []byte, dbVersion uint32) (*sstxRecord, error) {
	...
	case dbVersion < 3:
		...
		// Create and save the cdrutil.Tx of the read MsgTx and set its index.
		tx := cdrutil.NewTx(msgTx)
		tx.SetIndex(cdrutil.TxIndexUnknown)
	...
	case dbVersion >= 3:
		...
		return &sstxRecord{tx: cdrutil.NewTx(&tx), ts: time.Unix(unixTime, 0)}, nil
