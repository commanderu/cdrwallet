...
func TestStakeInvalidationOfTip(t *testing.T) {
	...
	block1Header := g.generate(cdrutil.BlockValid)
	block2Header := g.generate(cdrutil.BlockValid)
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		err = s.Adcdredit(ns, block1TxRec, nil, 0, false, 0)
		if err != nil {
			return err
		}
		...
		err = s.Adcdredit(ns, block2TxRec, nil, 0, false, 0)
		if err != nil {
			return err
		}
		...
		if bal.Total != cdrutil.Amount(block2Tx.TxOut[0].Value) {
			t.Errorf("Wrong balance: expected %v got %v", cdrutil.Amount(block2Tx.TxOut[0].Value), bal)
		}
		...
		if credits[0].Amount != cdrutil.Amount(block2Tx.TxOut[0].Value) {
		...
		*/
		...
		if bal.Total != cdrutil.Amount(block1Tx.TxOut[0].Value) {
			t.Errorf("Wrong balance: expected %v got %v", cdrutil.Amount(block1Tx.TxOut[0].Value), bal)
		}
		credits, err = s.UnspentOutputs(ns)
		if err != nil {
			return err
		...
		if credits[0].Amount != cdrutil.Amount(block1Tx.TxOut[0].Value) {
...
func TestStakeInvalidationTxInsert(t *testing.T) {
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		err = s.Adcdredit(ns, block1TxRec, makeBlockMeta(block1Header), 0, false, 0)
		if err != nil {
			return err
		}
		err = s.InsertMinedTx(ns, addrmgrNs, block2TxRec, &headerData[1].BlockHash)
		...
		err = s.Adcdredit(ns, block2TxRec, makeBlockMeta(block2Header), 0, false, 0)
		if err != nil {
			return err
		}
