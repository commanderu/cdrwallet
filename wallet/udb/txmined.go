...
type Store struct {
	...
	acctLookupFunc func(walletdb.ReadBucket, cdrutil.Address) (uint32, error)
}
...
func stakeValidate(ns walletdb.ReadWriteBucket, height int32) error {
	...
	for i := range blockRec.transactions {
		...
		for i, output := range txRec.MsgTx.TxOut {
			k, v := existsInvalidatecdredit(ns, &txRec.Hash, uint32(i), &blockRec.Block)
			if v == nil {
				continue
			}

			err = ns.NestedReadWriteBucket(bucketStakeInvalidatecdredits).
				Delete(k)
			if err != nil {
			...
			minedBalance += cdrutil.Amount(output.Value)
		}
		...
		for i := range txRec.MsgTx.TxIn {
			...
			_, err = spencdredit(ns, credKey, &debitIncidence)
			if err != nil {
				return err
			}
...
func (s *Store) fetchAccountForPkScript(addrmgrNs walletdb.ReadBucket,
	credVal []byte, unminecdredVal []byte, pkScript []byte) (uint32, error) {
	...
	if unminecdredVal != nil {
		acct, err := fetchRawUnminecdreditAccount(unminecdredVal)
		if err == nil {
			return acct, nil
		}
		storeErr, ok := err.(apperrors.E)
	...
	// Neither credVal or unminecdredVal were passed, or if they
...
func (s *Store) moveMinedTx(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, rec *TxRecord, recKey,
	...
	for i, input := range rec.MsgTx.TxIn {
		...
		debitIncidence.index = uint32(i)
		amt, err := spencdredit(ns, credKey, &debitIncidence)
		if err != nil {
			return err
		}
	...
	for i := uint32(0); i < uint32(len(rec.MsgTx.TxOut)); i++ {
		...
		// height/hash.
		amount, change, err := fetchRawUnminecdreditAmountChange(v)
		if err != nil {
			return err
		}
		...
		cred.amount = amount
		cred.change = change
		cred.opCode = fetchRawUnminecdreditTagOpcode(v)
		cred.isCoinbase = fetchRawUnminecdreditTagIsCoinbase(v)

		// Legacy credit output values may be of the wrong
		// size.
		scrType := fetchRawUnminecdreditScriptType(v)
		scrPos := fetchRawUnminecdreditScriptOffset(v)
		scrLen := fetchRawUnminecdreditScriptLength(v)
		...
		err = deleteRawUnminecdredit(ns, k)
		if err != nil {
			return err
		}
...
func (s *Store) InsertMinedTx(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, rec *TxRecord,
	...
	for i, input := range rec.MsgTx.TxIn {
		...
		if invalidated {
			...
			amt, err := spencdredit(ns, credKey, &spender)
			if err != nil {
				return err
			}
...
// Adcdredit marks a transaction record as containing a transaction output
...
func (s *Store) Adcdredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if invalidated {
		...
		cred := credit{
			...
			block:      block.Block,
			amount:     cdrutil.Amount(rec.MsgTx.TxOut[index].Value),
			change:     change,
			spentBy:    indexedIncidence{index: ^uint32(0)},
	...
	_, err := s.adcdredit(ns, rec, block, index, change, account)
	return err
}
...
func (s *Store) adcdredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if block == nil {
		...
		if existsRawUnminecdredit(ns, k) != nil {
			return false, nil
		}
		...
		v := valueUnminecdredit(cdrutil.Amount(rec.MsgTx.TxOut[index].Value),
			change, opCode, isCoinbase, hasExpiry, scrType, uint32(scrLoc),
			uint32(scrLen), account, DBVersion)
		return true, putRawUnminecdredit(ns, k, v)
	}
	...
	txOutAmt := cdrutil.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) addMultisigOut(ns walletdb.ReadWriteBucket, rec *TxRecord,
	...
	val = valueMultisigOut(p2shScriptHash,
		...
		block.Block.Hash,
		uint32(block.Block.Height),
		cdrutil.Amount(rec.MsgTx.TxOut[index].Value),
		*empty,     // Unspent
		0xFFFFFFFF, // Unspent
		rec.Hash)
...
func approvesParent(voteBits uint16) bool {
	return cdrutil.IsFlagSet16(voteBits, cdrutil.BlockValid)
}
...
func (s *Store) rollback(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, height int32) error {
	...
	for it.prev() {
		...
		removecdredits := make(map[string][]byte)
		...
		for i := range b.transactions {
			...
			for i, input := range rec.MsgTx.TxIn {
				...
				credVal := existsRawCredit(ns, credKey)
				if credVal == nil {
					credVal = removecdredits[string(credKey)]
				}
				if credVal == nil {
					return fmt.Errorf("missing credit value")
				}
			...
			for i, output := range rec.MsgTx.TxOut {
				...
				outPointKey := canonicalOutPoint(&rec.Hash, uint32(i))
				unminecdredVal := valueUnminecdredit(amt, change, opCode,
					isCoinbase, hasExpiry, scrType, uint32(scrLoc), uint32(scrLen),
					acct, DBVersion)
				err = putRawUnminecdredit(ns, outPointKey, unminecdredVal)
				if err != nil {
					return err
				}
...
func (s *Store) outputCreditInfo(ns walletdb.ReadBucket, op wire.OutPoint,
	...
	var minecdredV []byte
	unminecdredV := existsRawUnminecdredit(ns,
		canonicalOutPoint(&op.Hash, op.Index))
	if unminecdredV == nil {
		if block != nil {
			credK := keyCredit(&op.Hash, op.Index, block)
			minecdredV = existsRawCredit(ns, credK)
		}
	}
	if minecdredV == nil && unminecdredV == nil {
	...
	mined := false
	if unminecdredV != nil {
		amt, err = fetchRawUnminecdreditAmount(unminecdredV)
		if err != nil {
			return nil, err
		}

		opCode = fetchRawUnminecdreditTagOpcode(unminecdredV)
		isCoinbase = fetchRawCreditIsCoinbase(unminecdredV)
		hasExpiry = fetchRawCreditHasExpiry(unminecdredV, DBVersion)
		...
		scrLoc = fetchRawUnminecdreditScriptOffset(unminecdredV)
		scrLen = fetchRawUnminecdreditScriptLength(unminecdredV)
	}
	if minecdredV != nil {
		mined = true
		amt, err = fetchRawCreditAmount(minecdredV)
		if err != nil {
			return nil, err
		}

		opCode = fetchRawCreditTagOpCode(minecdredV)
		isCoinbase = fetchRawCreditIsCoinbase(minecdredV)
		hasExpiry = fetchRawCreditHasExpiry(minecdredV, DBVersion)

		// Same error caveat as above.
		scrLoc = fetchRawCreditScriptOffset(minecdredV)
		scrLen = fetchRawCreditScriptLength(minecdredV)
	}
...
func (s *Store) unspentOutputs(ns walletdb.ReadBucket) ([]*Credit, error) {
	...
	err = ns.NestedReadBucket(bucketUnminecdredits).ForEach(func(k, v []byte) error {
		if existsRawUnminedInput(ns, k) != nil {
			// Output is spent by an unmined transaction.
...
func (s *Store) UnspentOutpoints(ns walletdb.ReadBucket) ([]wire.OutPoint, error) {
	...
	err = ns.NestedReadBucket(bucketUnminecdredits).ForEach(func(k, v []byte) error {
		...
		opCode := fetchRawUnminecdreditTagOpcode(v)
		op.Tree = wire.TxTreeRegular
		if opCode != opNonstake {
			op.Tree = wire.TxTreeStake
		}
...
func (s *Store) UnspentMultisigCreditsForAddress(ns walletdb.ReadBucket,
	addr cdrutil.Address) ([]*MultisigCredit, error) {
...
func (s *Store) unspentMultisigCreditsForAddress(ns walletdb.ReadBucket,
	...
	if sha, ok := addr.(*cdrutil.AddressScriptHash); ok {
...
func (s *Store) UnspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket,
	amt cdrutil.Amount, height int32, minConf int32, all bool,
...
func (s *Store) fastCreditPkScriptLookup(ns walletdb.ReadBucket, credKey []byte,
	unminecdredKey []byte) ([]byte, error) {
	// It has to exists as a credit or an unmined credit.
	// Look both of these up. If it doesn't, throw an
	// error. Check unmined first, then mined.
	var minecdredV []byte
	unminecdredV := existsRawUnminecdredit(ns, unminecdredKey)
	if unminecdredV == nil {
		minecdredV = existsRawCredit(ns, credKey)
	}
	if minecdredV == nil && unminecdredV == nil {
	...
	if !mined {
		var op wire.OutPoint
		err := readCanonicalOutPoint(unminecdredKey, &op)
		if err != nil {
			return nil, err
		}
...
func (s *Store) unspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket, needed cdrutil.Amount,
	...
	if minConf == 0 {
		err = ns.NestedReadBucket(bucketUnminecdredits).ForEach(func(k, v []byte) error {
			...
			amt, err := fetchRawUnminecdreditAmount(v)
			if err != nil {
				return err
			}
...
type InputSource struct {
	source func(cdrutil.Amount) (cdrutil.Amount, []*wire.TxIn, [][]byte, error)
}
...
func (s *InputSource) SelectInputs(target cdrutil.Amount) (cdrutil.Amount, []*wire.TxIn, [][]byte, error) {
...
func (s *Store) MakeInputSource(ns, addrmgrNs walletdb.ReadBucket, account uint32, minConf, syncHeight int32) InputSource {
	...
	// created or not.
	var bucketUnspentCursor, bucketUnminecdreditsCursor walletdb.ReadCursor

	// Current inputs and their total value.  These are closed over by the
	// returned input source and reused across multiple calls.
	var (
		currentTotal   cdrutil.Amount
	...
	f := func(target cdrutil.Amount) (cdrutil.Amount, []*wire.TxIn, [][]byte, error) {
		...
		for currentTotal < target || target == 0 {
			var k, v []byte
			if bucketUnminecdreditsCursor == nil {
				b := ns.NestedReadBucket(bucketUnminecdredits)
				bucketUnminecdreditsCursor = b.ReadCursor()
				k, v = bucketUnminecdreditsCursor.First()
			} else {
				k, v = bucketUnminecdreditsCursor.Next()
			}
			if k == nil || v == nil {
				break
			}
...
func (s *Store) balanceFullScan(ns, addrmgrNs walletdb.ReadBucket, minConf int32,
	...
	err := ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		case txscript.OP_SSTX:
			...
			votingAuthorityAmt := cdrutil.Amount(0)
			lockedByTicketsAmt := cdrutil.Amount(0)
			...
			totalInputAmount := cdrutil.Amount(0)
	...
	err = ns.NestedReadBucket(bucketUnminecdredits).ForEach(func(k, v []byte) error {
		...
		utxoAmt, err := fetchRawUnminecdreditAmount(v)
		if err != nil {
			return err
		}
		...
		case txscript.OP_SSTX:
			txHash := extractRawUnminecdreditTxHash(k)
			...
			for _, txin := range rec.MsgTx.TxIn {
				...
				if rawUnmined != nil {
					...
					if int(txin.PreviousOutPoint.Index) < len(tx.TxOut) {
						totalInputAmount += cdrutil.Amount(tx.TxOut[txin.PreviousOutPoint.Index].Value)
					}
				} else {
...
type Balances struct {
	Account                 uint32
	ImmatureCoinbaseRewards cdrutil.Amount
	ImmatureStakeGeneration cdrutil.Amount
	LockedByTickets         cdrutil.Amount
	Spendable               cdrutil.Amount
	Total                   cdrutil.Amount
	VotingAuthority         cdrutil.Amount
	Unconfirmed             cdrutil.Amount
}
...
func (s *Store) TotalInput(dbtx walletdb.ReadTx, tx *wire.MsgTx) (cdrutil.Amount, error) {
	ns := dbtx.ReadBucket(wtxmgrBucketKey)

	var total cdrutil.Amount
	for _, in := range tx.TxIn {
		...
		total += cdrutil.Amount(tx.TxOut[in.PreviousOutPoint.Index].Value)
	}
