...
func (s *Store) minedTxDetails(ns walletdb.ReadBucket, txHash *chainhash.Hash, recKey, recVal []byte) (*TxDetails, error) {
	...
	credIter := makeReacdreditIterator(ns, recKey, DBVersion)
	for credIter.next() {
...
func (s *Store) unminedTxDetails(ns walletdb.ReadBucket, txHash *chainhash.Hash, v []byte) (*TxDetails, error) {
	...
	it := makeReadUnminecdreditIterator(ns, txHash, DBVersion)
	for it.next() {
	...
	for i, output := range details.MsgTx.TxIn {
		...
		v := existsRawUnminecdredit(ns, opKey)
		if v == nil {
			continue
		}
...
func (s *Store) ExistsUTXO(dbtx walletdb.ReadTx, op *wire.OutPoint) bool {
	...
	return existsRawUnminecdredit(ns, k) != nil
}
...
func (s *Store) rangeBlockTransactions(ns walletdb.ReadBucket, begin, end int32,
	...
	for advance(&blockIter) {
		...
		for _, txHash := range block.transactions {
			...
			credIter := makeReacdreditIterator(ns, k, DBVersion)
			for credIter.next() {
		...
		// commanderu: Block records are saved even when no transactions are
	...
	if block == nil {
		for _, input := range rec.MsgTx.TxIn {
			...
			if v != nil {
				...
				vUC := existsRawUnminecdredit(ns, k)
				if vUC == nil {
					continue
				}
				...
				// the script.
				scrPos := fetchRawUnminecdreditScriptOffset(vUC)
				scrLen := fetchRawUnminecdreditScriptLength(vUC)
