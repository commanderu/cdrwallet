...
func setup() error {
	...
	return walletdb.Update(db, func(dbtx walletdb.ReadWriteTx) error {
		...
		for count := 1; count < 4; count++ {
			...
			msgTx.AddTxOut(wire.NewTxOut(int64(cdrutil.Amount(1*count)), pkScript))
			...
			err = txmgr.Adcdredit(txmgrns, rec, blockMeta, 0, true, 0)
			if err != nil {
				return err
			}
		}
		...
		for count := 1; count < 4; count++ {
			faucetAddr, err := cdrutil.DecodeAddress("TsWjioPrP8E1TuTMmTrVMM2BA4iPrjQXBpR")
			if err != nil {
