...
import (
	...
	"github.com/commanderu/cdrutil"
	"github.com/commanderu/cdrutil/hdkeychain"
...
func setup() error {
	...
	return walletdb.Update(db, func(dbtx walletdb.ReadWriteTx) error {
		...
		wif, err := cdrutil.NewWIF(privKey, chainParams, chainec.ECTypeSecp256k1)
		if err != nil {
			return err
		}
		...
		for i := uint32(0); i < 6; i++ {
			...
			err = txmgr.Adcdredit(txmgrns, ticketRec, block, 0, false, udb.ImportedAddrAccount)
			if err != nil {
				return err
			}
			...
			err = txmgr.Adcdredit(txmgrns, spenderRec, block, 0, false, udb.ImportedAddrAccount)
			if err != nil {
				return err
			}
		}
...
func createUnsignedTicketPurchase(prevOut *wire.OutPoint,
	inputAmount, ticketPrice cdrutil.Amount) (*wire.MsgTx, error) {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addr,
		cdrutil.Amount(amountsCommitted[0]), ticketFeeLimits)
	if err != nil {
		return nil, err
	}
