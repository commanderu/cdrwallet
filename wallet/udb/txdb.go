...
func fetchMinedBalance(ns walletdb.ReadBucket) (cdrutil.Amount, error) {
	...
	return cdrutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt cdrutil.Amount) error {
	v := make([]byte, 8)
...
func extractRawUnminecdreditTxHash(k []byte) []byte {
	return k[:32]
}
...
func fetchRawCreditAmount(v []byte) (cdrutil.Amount, error) {
	if len(v) < 9 {
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (cdrutil.Amount, bool, error) {
	if len(v) < 9 {
	...
	return cdrutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (cdrutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read for raw credit amount change "+
			"(expected %d bytes, read %d)", bucketCredits, 9, len(v))
		return 0, false, storeError(apperrors.ErrData, str, nil)
	}
	return cdrutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spencdredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (cdrutil.Amount, error) {
	...
	return cdrutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (cdrutil.Amount, error) {
	...
	return cdrutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func existsInvalidatecdredit(ns walletdb.ReadBucket, txHash *chainhash.Hash, index uint32, block *Block) (k, v []byte) {
	k = keyCredit(txHash, index, block)
	v = ns.NestedReadBucket(bucketStakeInvalidatecdredits).Get(k)
	return
}
...
func makeReacdreditIterator(ns walletdb.ReadBucket, prefix []byte, dbVersion uint32) creditIterator {
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = cdrutil.Amount(byteOrder.Uint64(it.cv))
...
func valueDebit(amount cdrutil.Amount, credKey []byte) []byte {
	v := make([]byte, 80)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount cdrutil.Amount, block *Block, credKey []byte) error {
...
func extractRawDebitAmount(v []byte) cdrutil.Amount {
	return cdrutil.Amount(byteOrder.Uint64(v[:8]))
}
...
func valueUnminecdredit(amount cdrutil.Amount, change bool, opCode uint8,
...
func putRawUnminecdredit(ns walletdb.ReadWriteBucket, k, v []byte) error {
	err := ns.NestedReadWriteBucket(bucketUnminecdredits).Put(k, v)
	if err != nil {
...
func fetchRawUnminecdreditIndex(k []byte) (uint32, error) {
...
func fetchRawUnminecdreditAmount(v []byte) (cdrutil.Amount, error) {
...
func fetchRawUnminecdreditAmountChange(v []byte) (cdrutil.Amount, bool, error) {
	if len(v) < unconfValueSizeLegacy {
		str := "short unmined credit value when look up credit amt change"
		return 0, false, storeError(apperrors.ErrData, str, nil)
	}
	amt := cdrutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
	return amt, change, nil
}

func fetchRawUnminecdreditTagOpcode(v []byte) uint8 {
	return (((v[8] >> 2) & 0x07) + 0xb9)
}

func fetchRawUnminecdreditTagIsCoinbase(v []byte) bool {
	return v[8]&(1<<5) != 0
}

func fetchRawUnminecdreditScriptType(v []byte) scriptType {
...
func fetchRawUnminecdreditScriptOffset(v []byte) uint32 {
	if len(v) < unconfValueSize {
		return 0
	}
...
func fetchRawUnminecdreditScriptLength(v []byte) uint32 {
	if len(v) < unconfValueSize {
		return 0
	}
...
func fetchRawUnminecdreditAccount(v []byte) (uint32, error) {
	if len(v) < unconfValueSize {
...
func existsRawUnminecdredit(ns walletdb.ReadBucket, k []byte) []byte {
	return ns.NestedReadBucket(bucketUnminecdredits).Get(k)
}

func deleteRawUnminecdredit(ns walletdb.ReadWriteBucket, k []byte) error {
	err := ns.NestedReadWriteBucket(bucketUnminecdredits).Delete(k)
	if err != nil {
...
// unminecdreditIterator allows for cursor iteration over all credits, in order,
// from a single unmined transaction.
//
//  Example usage:
//
//   it := makeUnminecdreditIterator(ns, txHash)
//   for it.next() {
...
type unminecdreditIterator struct {
	c         walletdb.ReadWriteCursor
	dbVersion uint32
	prefix    []byte
	ck        []byte
...
func makeReadUnminecdreditIterator(ns walletdb.ReadBucket, txHash *chainhash.Hash, dbVersion uint32) unminecdreditIterator {
	c := ns.NestedReadBucket(bucketUnminecdredits).ReadCursor()
	return unminecdreditIterator{c: readCursor{c}, prefix: txHash[:], dbVersion: dbVersion}
}

func (it *unminecdreditIterator) readElem() error {
	index, err := fetchRawUnminecdreditIndex(it.ck)
	if err != nil {
		return err
	}
	amount, change, err := fetchRawUnminecdreditAmountChange(it.cv)
	if err != nil {
		return err
	}
...
func (it *unminecdreditIterator) next() bool {
	if it.c == nil {
		return false
	}
...
// unavailable until https://github.com/boltdb/bolt/issues/620 is fixed.
// func (it *unminecdreditIterator) delete() error {
// 	err := it.c.Delete()
// 	if err != nil {
// 		str := "failed to delete unmined credit"
// 		return storeError(apperrors.ErrDatabase, str, err)
...
func (it *unminecdreditIterator) reposition(txHash *chainhash.Hash, index uint32) {
...
func valueMultisigOut(sh [ripemd160.Size]byte, m uint8, n uint8,
	...
	blockHeight uint32, amount cdrutil.Amount, spentBy chainhash.Hash,
...
func fetchMultisigOut(k, v []byte) (*MultisigOut, error) {
	...
	mso.Amount = cdrutil.Amount(byteOrder.Uint64(v[59:67]))
...
func fetchMultisigOutAmount(v []byte) cdrutil.Amount {
	return cdrutil.Amount(byteOrder.Uint64(v[59:67]))
}
...
func createStore(ns walletdb.ReadWriteBucket, chainParams *chaincfg.Params) error {
	...
	_, err = ns.CreateBucket(bucketUnminecdredits)
	if err != nil {
	...
	_, err = ns.CreateBucket(bucketStakeInvalidatecdredits)
	if err != nil {
