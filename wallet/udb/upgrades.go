...
const (
	...
	// assigned to commanderu in SLIP0044.  The upgrade does not add or remove any
...
func lastUsedAddressIndexUpgrade(tx walletdb.ReadWriteTx, publicPassphrase []byte) error {
	...
	for account := uint32(0); account <= lastAccount; account++ {
		...
		for child := uint32(0); child < hdkeychain.HardenedKeyStart; child++ {
			...
			// cdrutil.NewAddressPubKeyHash.  Also, while it looks like a
...
func hasExpiryUpgrade(tx walletdb.ReadWriteTx, publicPassphrase []byte) error {
	...
	unminecdreditsBucket := txmgrBucket.NestedReadWriteBucket(bucketUnminecdredits)
	unminedCursor := unminecdreditsBucket.ReadWriteCursor()
	unminecdreditsKV := map[string][]byte{}
	for k, v := unminedCursor.First(); v != nil; k, v = unminedCursor.Next() {
		hash, err := chainhash.NewHash(extractRawUnminecdreditTxHash(k))
		if err != nil {
			return err
		}
		...
		if record.MsgTx.Expiry != wire.NoExpiryValue {
			...
			vCpy[8] |= 1 << 4
			unminecdreditsKV[string(k)] = vCpy
		}
	}

	for k, v := range unminecdreditsKV {
		err = unminecdreditsBucket.Put([]byte(k), v)
		if err != nil {
			return err
		}
	}
...
func hasExpiryFixedUpgrade(tx walletdb.ReadWriteTx, publicPassphrase []byte) error {
	...
	// Iterate through all unmined credits
	unminecdreditsBucket := txmgrBucket.NestedReadWriteBucket(bucketUnminecdredits)
	unminedCursor := unminecdreditsBucket.ReadCursor()
	unminecdreditsKV := map[string][]byte{}
	for k, v := unminedCursor.First(); v != nil; k, v = unminedCursor.Next() {
		hash, err := chainhash.NewHash(extractRawUnminecdreditTxHash(k))
		if err != nil {
		...
		if record.MsgTx.Expiry != wire.NoExpiryValue {
			...
			// Reset OP_SSTXCHANGE flag if this is a ticket purchase
			// OP_SSTXCHANGE output.
			idx, err := fetchRawUnminecdreditIndex(k)
			if err != nil {
				return err
			}
