...
func TestUpgrades(t *testing.T) {
	...
	d, err := ioutil.TempDir("", "cdrwallet_udb_TestUpgrades")
	if err != nil {
		t.Fatal(err)
	}
...
func verifyV8Upgrade(t *testing.T, db walletdb.DB) {
	err := walletdb.View(db, func(tx walletdb.ReadTx) error {
		...
		unminecdreditBucket := ns.NestedReadBucket(bucketUnminecdredits)
		err = unminecdreditBucket.ForEach(func(k []byte, v []byte) error {
			hasExpiry := fetchRawCreditHasExpiry(v, DBVersion)
