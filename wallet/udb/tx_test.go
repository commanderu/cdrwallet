...
// Copyright (c) 2015 The commanderu developers
...
import (
	...
	_ "github.com/commanderu/cdrwallet/walletdb/bdb"
)
...
var (
	...
	TstRecvTx, _                    = cdrutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = cdrutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *cdrutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, err := cdrutil.NewTxFromBytesLegacy(TstRecvSerializedTx)
	if err != nil {
		t.Errorf("failed to deserialize test transaction: %v", err.Error())
		return
	}

	// Switch txout amount to 1 cdr.  Transaction store doesn't
	...
	TstDoubleSpendTx := cdrutil.NewTx(newDupMsgTx)
	TstDoubleSpendSerializedTx := serializeTx(TstDoubleSpendTx)

	// Create a "signed" (with invalid sigs) tx that spends output 0 of
	// the double spend.
	spendingTx := wire.NewMsgTx()
	spendingTxIn := wire.NewTxIn(wire.NewOutPoint(TstDoubleSpendTx.Sha(), 0, cdrutil.TxTreeRegular), []byte{0, 1, 2, 3, 4})
	...
	TstSpendingTx := cdrutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			f: func(s *Store) (*Store, error) {
				...
				err = s.Adcdredit(rec, nil, 1, false, defaultAccount)
				return s, err
			},
			bal: 0,
			unc: cdrutil.Amount(TstRecvTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Sha(),
					Index: 1,
					Tree:  cdrutil.TxTreeRegular,
				}: {},
			},
			unmined: map[chainhash.Hash]struct{}{
				*TstRecvTx.Sha(): {},
		...
		},
		{
			...
			f: func(s *Store) (*Store, error) {
				...
				err = s.Adcdredit(rec, TstRecvTxBlockDetails, 1, false,
					defaultAccount)
				return s, err
			},
			bal: cdrutil.Amount(TstRecvTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Sha(),
		...
		},
		{
			...
			bal: cdrutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			f: func(s *Store) (*Store, error) {
				...
				err = s.Adcdredit(rec, nil, 0, true, defaultAccount)
				return s, err
			},
			bal: 0,
			unc: cdrutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			f: func(s *Store) (*Store, error) {
				...
				err = s.Adcdredit(rec, nil, 1, true, defaultAccount)
				return s, err
			},
			bal: 0,
			unc: cdrutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Sha(),
					Index: 0,
		...
		},
		{
			...
			bal: cdrutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Sha(),
		...
		},
		{
			...
			unc: cdrutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				*wire.NewOutPoint(TstSpendingTx.Sha(), 0, cdrutil.TxTreeRegular): {},
				*wire.NewOutPoint(TstSpendingTx.Sha(), 1, cdrutil.TxTreeRegular): {},
			},
			unmined: map[chainhash.Hash]struct{}{
		...
		},
		{
			...
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				*wire.NewOutPoint(TstRecvTx.Sha(), 1, cdrutil.TxTreeRegular): {},
			},
			unmined: map[chainhash.Hash]struct{}{},
		},
	}
...
func TestFindingSpentCredits(t *testing.T) {
	...
	defaultAccount := uint32(0)
	err = s.Adcdredit(recvRec, TstRecvTxBlockDetails, 1, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = s.Adcdredit(spendingRec, TstSignedTxBlockDetails, 0, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
	...
	expectedBal := cdrutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
	...
	op := wire.NewOutPoint(TstSpendingTx.Sha(), 0, cdrutil.TxTreeStake)
...
func TestCoinbases(t *testing.T) {
	...
	defaultAccount := uint32(0)
	err = s.Adcdredit(cbRec, &b100, 0, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
	err = s.Adcdredit(cbRec, &b100, 2, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}

	// Balance should be 0 if the coinbase is immature, 50 cdr at and beyond
	// maturity.
	//
	...
	err = s.Adcdredit(spenderARec, nil, 0, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = s.Adcdredit(spenderBRec, &bMaturity, 0, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
	for i, tst := range balTests {
...
func TestMoveMultipleToSameBlock(t *testing.T) {
	...
	err = s.Adcdredit(cbRec, &b100, 1, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = s.Adcdredit(spenderARec, nil, 1, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
	spenderBTime := time.Now()
	...
	err = s.Adcdredit(spenderBRec, nil, 0, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
	err = s.Adcdredit(spenderBRec, nil, 1, false, defaultAccount)
	if err != nil {
		t.Fatal(err)
	}
