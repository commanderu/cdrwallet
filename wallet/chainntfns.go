...
func (w *Wallet) evaluateStakePoolTicket(rec *udb.TxRecord,
	blockHeight int32, poolUser cdrutil.Address) (bool, error) {
	tx := rec.MsgTx
	...
	out := cdrutil.Amount(0)
	for i := range tx.TxOut {
		out += cdrutil.Amount(tx.TxOut[i].Value)
	}
	fees := in - out
	...
	if exists {
		...
		feeNeeded := txrules.StakePoolTicketFee(cdrutil.Amount(
...
func (w *Wallet) processTransactionRecord(dbtx walletdb.ReadWriteTx, rec *udb.TxRecord,
	...
	if stake.IsSStx(&rec.MsgTx) {
		...
		if insert {
			err := w.StakeMgr.InsertSStx(stakemgrNs, cdrutil.NewTx(&rec.MsgTx))
			if err != nil {
				...
				if err != nil {
					...
					if n, err := w.NetworkBackend(); err == nil {
						...
						err := n.LoadTxFilter(context.TODO(),
							false, []cdrutil.Address{addr}, nil)
						if err != nil {
							return err
						}
					}
	...
	for i, output := range rec.MsgTx.TxOut {
		...
		for _, addr := range addrs {
			ma, err := w.Manager.Address(addrmgrNs, addr)
			if err == nil {
				err = w.TxStore.Adcdredit(txmgrNs, rec, blockMeta,
...
func (w *Wallet) VoteOnOwnedTickets(winningTicketHashes []*chainhash.Hash,
	...
	for i, vote := range votes {
		...
		if err != nil {
			rpcErr, ok := err.(*cdrjson.RPCError)
			if ok {
				if rpcErr.Code != cdrjson.ErrRPCDuplicateTx {
...
func (w *Wallet) RevokeOwnedTickets(missedTicketHashes []*chainhash.Hash) error {
	...
	err = walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, ticketHash := range ticketHashes {
			...
			err = w.checkHighFees(cdrutil.Amount(ticketPurchase.TxOut[0].Value), revocation)
			if err != nil {
