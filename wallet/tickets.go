...
func (w *Wallet) LiveTicketHashes(chainClient *cdrrpcclient.Client, includeImmature bool) ([]chainhash.Hash, error) {
	...
	// these transactions are not part of the wallet's transaction history, cdr
	// must be queried for their blockchain height.  This functionality requires
	// the cdr transaction index to be enabled.
...
// pruned tickets.
func (w *Wallet) TicketHashesForVotingAddress(votingAddr cdrutil.Address) ([]chainhash.Hash, error) {
	var ticketHashes []chainhash.Hash
...
func (w *Wallet) updateStakePoolInvalidTicket(stakemgrNs walletdb.ReadWriteBucket,
	addr cdrutil.Address, ticket *chainhash.Hash) error {
...
func (w *Wallet) AddTicket(ticket *wire.MsgTx) error {
	...
	return walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		err := w.StakeMgr.InsertSStx(stakemgrNs, cdrutil.NewTx(ticket))
		if err != nil {
			return err
		}
...
// revocations.
func (w *Wallet) RevokeTickets(chainClient *cdrrpcclient.Client) error {
	var ticketHashes []chainhash.Hash
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
