...
import (
	...
	"github.com/commanderu/cdr/blockchain"
	...
	"github.com/commanderu/cdr/cdrec/secp256k1"
...
type Wallet struct {
	...
	votingEnabled      bool
	balanceToMaintain  cdrutil.Amount
	poolAddress        cdrutil.Address
	poolFees           float64
	...
	ticketFeeIncrement     cdrutil.Amount
	DisallowFree           bool
	AllowHighFees          bool
...
func (w *Wallet) StakeDifficulty() (cdrutil.Amount, error) {
...
func (w *Wallet) BalanceToMaintain() cdrutil.Amount {
...
func (w *Wallet) SetBalanceToMaintain(balance cdrutil.Amount) {
	w.stakeSettingsLock.Lock()
...
func (w *Wallet) RelayFee() cdrutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee cdrutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) TicketFeeIncrement() cdrutil.Amount {
...
func (w *Wallet) SetTicketFeeIncrement(fee cdrutil.Amount) {
	w.ticketFeeIncrementLock.Lock()
...
func (w *Wallet) loadActiveAddrs(dbtx walletdb.ReadTx, nb NetworkBackend) (uint64, error) {
	...
	loadBranchAddrs := func(branchKey *hdkeychain.ExtendedKey, n uint32, errs chan<- error) {
		...
		for child := uint32(0); child <= n; child += step {
			child := child
			g.Go(func() error {
				addrs := make([]cdrutil.Address, 0, step)
...
func (w *Wallet) CommittedTickets(tickets []*chainhash.Hash) ([]*chainhash.Hash, []cdrutil.Address, error) {
	hashes := make([]*chainhash.Hash, 0, len(tickets))
	addresses := make([]cdrutil.Address, 0, len(tickets))
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for _, v := range tickets {
			...
			var bestAddr cdrutil.Address
			var bestAmount cdrutil.Amount
			...
			for i := 1; i < len(tx.TxOut); i += 2 {
				...
				if _, ok := addr.(*cdrutil.AddressPubKeyHash); !ok {
...
func (w *Wallet) FetchHeaders(n NetworkBackend) (count int, rescanFrom chainhash.Hash, rescanFromHeight int32,
	...
	//
	// See https://github.com/commanderu/cdr/issues/427 for details.  This hack
...
type (
	...
	createMultisigTxRequest struct {
		...
		pubkeys   []*cdrutil.AddressSecpPubKey
		nrequired int8
		minconf   int32
		resp      chan createMultisigTxResponse
	}
	purchaseTicketRequest struct {
		minBalance  cdrutil.Amount
		spendLimit  cdrutil.Amount
		minConf     int32
		ticketAddr  cdrutil.Address
		account     uint32
		numTickets  int
		poolAddress cdrutil.Address
		poolFees    float64
		expiry      int32
		txFee       cdrutil.Amount
		ticketFee   cdrutil.Amount
...
func (w *Wallet) Consolidate(inputs int, account uint32,
	address cdrutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) CreateMultisigTx(account uint32, amount cdrutil.Amount,
	pubkeys []*cdrutil.AddressSecpPubKey, nrequired int8,
	minconf int32) (*CreatedTx, cdrutil.Address, []byte, error) {
...
// tickets.
func (w *Wallet) PurchaseTickets(minBalance, spendLimit cdrutil.Amount,
	minConf int32, ticketAddr cdrutil.Address, account uint32,
	numTickets int, poolAddress cdrutil.Address, poolFees float64,
	expiry int32, txFee cdrutil.Amount, ticketFee cdrutil.Amount) ([]*chainhash.Hash,
	error) {
...
// spending to it in the blockchain or cdr mempool), the next chained address
// is returned.
func (w *Wallet) CurrentAddress(account uint32) (cdrutil.Address, error) {
...
func (w *Wallet) PubKeyForAddress(a cdrutil.Address) (chainec.PublicKey, error) {
...
func (w *Wallet) SignMessage(msg string, addr cdrutil.Address) (sig []byte, err error) {
	var buf bytes.Buffer
	wire.WriteVarString(&buf, 0, "commanderu Signed Message:\n")
...
func VerifyMessage(msg string, addr cdrutil.Address, sig []byte) (bool, error) {
	...
	recoveredAddr, err := cdrutil.NewAddressSecpPubKey(serializedPK, addr.Net())
	if err != nil {
		return false, err
	}
...
func (w *Wallet) HaveAddress(a cdrutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a cdrutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a cdrutil.Address) (udb.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *udb.TxDetails, addrMgr *udb.Manager,
	syncHeight int32, net *chaincfg.Params) (sends, receives []cdrjson.ListTransactionsResult) {
	...
	txTypeStr := cdrjson.LTTTRegular
	switch details.TxType {
	case stake.TxTypeSStx:
		txTypeStr = cdrjson.LTTTTicket
	case stake.TxTypeSSGen:
		txTypeStr = cdrjson.LTTTVote
	case stake.TxTypeSSRtx:
		txTypeStr = cdrjson.LTTTRevocation
	}

	// Fee can only be determined if every input is a debit.
	var feeF64 float64
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal cdrutil.Amount
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := cdrutil.Amount(output.Value).ToCoin()
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]cdrjson.ListTransactionsResult, error) {
	txList := []cdrjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]cdrjson.ListTransactionsResult, error) {
	txList := []cdrjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]cdrjson.ListTransactionsResult, error) {
	txList := []cdrjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*cdrutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]cdrjson.ListTransactionsResult, error) {
	txList := []cdrjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) GetTickets(f func([]*TicketSummary, *wire.BlockHeader) (bool, error), chainClient *cdrrpcclient.Client, startBlock, endBlock *BlockIdentifier) error {
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*cdrutil.Amount)
...
func (w *Wallet) ListUnspent(minconf, maxconf int32, addresses map[string]struct{}) ([]*cdrjson.ListUnspentResult, error) {
	var results []*cdrjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		for i := range unspent {
			...
			case stake.TxTypeSStx:
				...
				// ticket maturity + 1??? Check this commanderu TODO
				if output.Index == 0 {
		...
		scSwitch:
			...
			result := &cdrjson.ListUnspentResult{
...
func (w *Wallet) DumpWIFPrivateKey(addr cdrutil.Address) (string, error) {
	var privKey chainec.PrivateKey
	var done func()
	defer func() {
		if done != nil {
	...
	wif, err := cdrutil.NewWIF(privKey, w.chainParams, privKey.GetType())
	if err != nil {
		return "", err
	}
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *cdrutil.WIF) (string, error) {
	...
	if n, err := w.NetworkBackend(); err == nil {
		err := n.LoadTxFilter(context.TODO(), false, []cdrutil.Address{addr}, nil)
		if err != nil {
...
// a P2SH address.
func (w *Wallet) RedeemScriptCopy(addr cdrutil.Address) ([]byte, error) {
	var scriptCopy []byte
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
...
func (w *Wallet) StakeInfo(chainClient *cdrrpcclient.Client) (*StakeInfoData, error) {
	// This is only needed for the total count and can be optimized.
	mempoolTicketsFuture := chainClient.GetRawMempoolAsync(cdrjson.GRMTickets)
		...
		for it.Next() {
			...
			if it.SpenderHash != (chainhash.Hash{}) {
				...
				case isVote(spender):
					...
					// earned by fees.
					res.TotalSubsidy += cdrutil.Amount(spender.TxIn[0].ValueIn)
...
func (w *Wallet) LockedOutpoints() []cdrjson.TransactionInput {
	locked := make([]cdrjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = cdrjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) SortedActivePaymentAddresses() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr cdrutil.Address) error {
...
type AccountTotalReceivedResult struct {
	...
	AccountName      string
	TotalReceived    cdrutil.Amount
	LastConfirmation int32
}

// TotalReceivedForAccounts iterates through a wallet's transaction history,
// returning the total amount of commanderu received for all accounts.
...
// returning the total amount of commanderu received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr cdrutil.Address, minConf int32) (cdrutil.Amount, error) {
	var amount cdrutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*cdrutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr cdrutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr cdrutil.Address) ([]byte, error) {
...
// address.
func (w *Wallet) CreateSignature(tx *wire.MsgTx, idx uint32, addr cdrutil.Address,
...
func Open(cfg *Config) (*Wallet, error) {
	...
	// Amounts
	w.ticketFeeIncrement, err = cdrutil.NewAmount(cfg.TicketFee)
	if err != nil {
		return nil, err
	}
	w.relayFee, err = cdrutil.NewAmount(cfg.RelayFee)
	if err != nil {
		return nil, err
	}
