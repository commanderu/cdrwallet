...
const DefaultRelayFeePerKb cdrutil.Amount = 1e5

// IsDustAmount determines whether a transaction output value and script length would
// cause the output to be considered dust.  Transactions with dust outputs are
// not standard and are rejected by mempools with default policies.
func IsDustAmount(amount cdrutil.Amount, scriptSize int, relayFeePerKb cdrutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb cdrutil.Amount) bool {
	...
	return IsDustAmount(cdrutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb cdrutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > cdrutil.MaxAmount {
...
func FeeForSerializeSize(relayFeePerKb cdrutil.Amount, txSerializeSize int) cdrutil.Amount {
	fee := relayFeePerKb * cdrutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > cdrutil.MaxAmount {
		fee = cdrutil.MaxAmount
	}
...
func PaysHighFees(totalInput cdrutil.Amount, tx *wire.MsgTx) bool {
