...
// NetworkBackend provides wallets with commanderu network functionality.  Some
...
type NetworkBackend interface {
	...
	LoadTxFilter(ctx context.Context, reload bool, addrs []cdrutil.Address, outpoints []wire.OutPoint) error
	PublishTransaction(ctx context.Context, tx *wire.MsgTx) error

	// Tricky but not impossible for spv
	AddressesUsed(ctx context.Context, addrs []cdrutil.Address) (bitset.Bytes, error)
	Rescan(ctx context.Context, blocks []chainhash.Hash) ([]*RescannedBlock, error)

	// TODO: these should be known directly by the wallet.
	StakeDifficulty(ctx context.Context) (cdrutil.Amount, error)
