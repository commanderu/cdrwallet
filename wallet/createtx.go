...
import (
	...
	"github.com/commanderu/cdr/mempool"
...
// account.
func (w *Wallet) NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb cdrutil.Amount, account uint32, minConf int32,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		case OutputSelectionAlgorithmAll:
			...
			inputSource = func(cdrutil.Amount) (cdrutil.Amount, []*wire.TxIn, [][]byte, error) {
				total, inputs, prevScripts, err := sourceImpl.SelectInputs(cdrutil.MaxAmount)
				switch err.(type) {
...
func (s *secretSource) GetKey(addr cdrutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr cdrutil.Address) ([]byte, error) {
...
func (w *Wallet) insertIntoTxMgr(ns walletdb.ReadWriteBucket, msgTx *wire.MsgTx) (*udb.TxRecord, error) {
	...
	if err != nil {
		return nil, cdrjson.ErrInternal
	}
	...
	for i, output := range msgTx.TxOut {
		...
		for _, addr := range addrs {
			...
			if err == nil {
				...
				err = w.TxStore.Adcdredit(txmgrNs, rec, nil,
...
func (w *Wallet) checkHighFees(totalInput cdrutil.Amount, tx *wire.MsgTx) error {
	if w.AllowHighFees {
		return nil
	}
...
//
// commanderu: This func also sends the transaction, and if successful, inserts it
...
func (w *Wallet) txToOutputsInternal(outputs []*wire.TxOut, account uint32, minconf int32,
	n NetworkBackend, randomizeChangeIdx bool, txFee cdrutil.Amount) (*txauthor.AuthoredTx, error) {
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := cdrutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisigInternal(dbtx walletdb.ReadWriteTx, account uint32,
	amount cdrutil.Amount, pubkeys []*cdrutil.AddressSecpPubKey, nRequired int8,
	minconf int32) (*CreatedTx, cdrutil.Address, []byte, error) {
	...
	txToMultisigError :=
		func(err error) (*CreatedTx, cdrutil.Address, []byte, error) {
	...
	// fee estimator.
	var feeEstForTx cdrutil.Amount
	switch {
	...
	scAddr, err := cdrutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(err)
	}
	...
	// Request updates from cdr for new transactions sent to this
	// script hash address.
	utilAddrs := make([]cdrutil.Address, 1)
	utilAddrs[0] = scAddr
	err = n.LoadTxFilter(context.TODO(), false, []cdrutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(err)
	}
...
func (w *Wallet) compressWallet(maxNumIns int, account uint32, changeAddr cdrutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr cdrutil.Address) (*chainhash.Hash, error) {
...
func makeTicket(params *chaincfg.Params, inputPool *extendedOutPoint,
	input *extendedOutPoint, addrVote cdrutil.Address, addrSubsidy cdrutil.Address,
	ticketCost int64, addrPool cdrutil.Address) (*wire.MsgTx, error) {
	mtx := wire.NewMsgTx()
	...
	zeroed := [20]byte{}
	addrZeroed, err := cdrutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return nil, err
	}
	...
	if addrPool != nil {
		pkScript, err = txscript.GenerateSStxAddrPush(addrPool,
			cdrutil.Amount(amountsCommitted[0]), limits)
		if err != nil {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addrSubsidy,
		cdrutil.Amount(amountsCommitted[userSubsidyNullIdx]), limits)
	if err != nil {
...
func (w *Wallet) purchaseTickets(req purchaseTicketRequest) ([]*chainhash.Hash, error) {
	...
	if w.addressReuse {
		...
		addrFunc = func(persistReturnedChildFunc, uint32) (cdrutil.Address, error) {
			return addr, err
		}
	}
	...
	var neededPerTicket, ticketFee cdrutil.Amount
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * cdrutil.Amount(req.numTickets)
	...
	for i := 0; i < req.numTickets; i++ {
		...
		// an address.
		var addrVote, addrSubsidy cdrutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       cdrutil.Amount(eopPool.amt),
		...
		err = w.checkHighFees(cdrutil.Amount(eop.amt), ticket)
		if err != nil {
			return nil, err
		}
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount cdrutil.Amount, currentHeight int32) ([]udb.Credit, error) {
...
func (w *Wallet) signVoteOrRevocation(addrmgrNs walletdb.ReadBucket, ticketPurchase, tx *wire.MsgTx, isVote bool) error {
	...
	// can be performed.
	var getKey txscript.KeyClosure = func(addr cdrutil.Address) (chainec.PrivateKey, bool, error) {
	...
	var getScript txscript.ScriptClosure = func(addr cdrutil.Address) ([]byte, error) {
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB cdrutil.Amount) (*wire.MsgTx, error) {
	// Parse the ticket purchase transaction to determine the required output
	// destinations for vote rewards or revocations.
	ticketPayKinds, ticketHash160s, ticketValues, _, _, _ :=
		stake.TxSStxStakeOutputInfo(ticketPurchase)
	...
	for _, output := range revocation.TxOut {
		if cdrutil.Amount(output.Value) > feeEstimate {
			amount := cdrutil.Amount(output.Value) - feeEstimate
