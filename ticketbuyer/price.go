...
func (t *TicketPurchaser) calcAverageTicketPrice(height int64) (cdrutil.Amount, error) {
	...
	// these two prices.
	var avgPricePoolAmt cdrutil.Amount
	if t.priceMode == AvgPricePoolMode || t.priceMode == AvgPriceDualMode {
		poolValue, err := t.cdrChainSvr.GetTicketPoolValue()
		if err != nil {
			return 0, err
		}
		bestBlockH, err := t.cdrChainSvr.GetBestBlockHash()
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.cdrChainSvr.GetBlockHeader(bestBlockH)
		if err != nil {
			return 0, err
		}
		...
		avgPricePoolAmt = poolValue / cdrutil.Amount(poolSize)
	...
	if t.priceMode == AvgPriceVWAPMode || t.priceMode == AvgPriceDualMode {
		...
		for i := 0; i < vwapReqTries; i++ {
			ticketVWAP, err = t.cdrChainSvr.TicketVWAP(&startVWAPHeight,
				&endVWAPHeight)
