...
type diffPeriodFee struct {
	difficulty cdrutil.Amount
	difference cdrutil.Amount // Difference from current difficulty
	fee        cdrutil.Amount
}
...
func (t *TicketPurchaser) findClosestFeeWindows(difficulty cdrutil.Amount,
	useMedian bool) (cdrutil.Amount, error) {
	wtcUint32 := uint32(windowsToConsider)
	info, err := t.cdrChainSvr.TicketFeeInfo(&zeroUint32, &wtcUint32)
	if err != nil {
		return 0.0, err
	}
	...
	for i := range info.FeeInfoWindows {
		...
		blH, err := t.cdrChainSvr.GetBlockHash(startHeight)
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.cdrChainSvr.GetBlockHeader(blH)
		if err != nil {
			return 0, err
		}

		windowDiffAmt := cdrutil.Amount(blkHeader.SBits)

		var fee cdrutil.Amount
		if !useMedian {
			fee, err = cdrutil.NewAmount(info.FeeInfoWindows[i].Mean)
		} else {
			fee, err = cdrutil.NewAmount(info.FeeInfoWindows[i].Median)
		}
		if err != nil {
			return 0, err
		}
...
func (t *TicketPurchaser) findTicketFeeBlocks(useMedian bool) (cdrutil.Amount, error) {
	btaUint32 := uint32(t.cfg.BlocksToAvg)
	info, err := t.cdrChainSvr.TicketFeeInfo(&btaUint32, nil)
	if err != nil {
		return 0.0, err
	}

	var sum, tmp cdrutil.Amount
	for i := range info.FeeInfoBlocks {
		if !useMedian {
			tmp, err = cdrutil.NewAmount(info.FeeInfoBlocks[i].Mean)
		} else {
			tmp, err = cdrutil.NewAmount(info.FeeInfoBlocks[i].Median)
		}
		if err != nil {
			return 0, err
		}
	...
	return sum / cdrutil.Amount(t.cfg.BlocksToAvg), nil
}
