...
func (t *TicketPurchaser) ownTicketsInMempool() (int, error) {
	...
	if t.votingAddress != nil {
		tiHashes, err := t.cdrChainSvr.GetRawMempool(cdrjson.GRMTickets)
		if err != nil {
			return 0, err
		}
		...
		for i := range tiHashes {
			raw, err := t.cdrChainSvr.GetRawTransactionVerbose(tiHashes[i])
			if err != nil {
				return 0, err
			}
			...
			addr, err := cdrutil.DecodeAddress(addrStr)
			if err != nil {
				return 0, err
			}
...
func (t *TicketPurchaser) allTicketsInMempool() (int, error) {
	tfi, err := t.cdrChainSvr.TicketFeeInfo(&zeroUint32, &zeroUint32)
	if err != nil {
		return 0, err
	}
