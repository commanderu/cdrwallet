...
type TicketPurchaser struct {
	...
	cdrChainSvr     *cdrrpcclient.Client
...
func (t *TicketPurchaser) BalanceToMaintain() cdrutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetBalanceToMaintain(balanceToMaintain int64) {
	t.purchaserMtx.Lock()
	t.balanceToMaintain = cdrutil.Amount(balanceToMaintain)
	t.purchaserMtx.Unlock()
}

// MaxPriceAbsolute returns the max absolute price to purchase a ticket.
func (t *TicketPurchaser) MaxPriceAbsolute() cdrutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetMaxPriceAbsolute(maxPriceAbsolute int64) {
	t.purchaserMtx.Lock()
	t.maxPriceAbsolute = cdrutil.Amount(maxPriceAbsolute)
	t.purchaserMtx.Unlock()
}
...
func (t *TicketPurchaser) MaxFee() cdrutil.Amount {
	t.purchaserMtx.Lock()
	maxFee := t.maxFee
	t.purchaserMtx.Unlock()
	return maxFee
...
func (t *TicketPurchaser) SetMaxFee(maxFee int64) {
	t.purchaserMtx.Lock()
	t.maxFee = cdrutil.Amount(maxFee)
	t.purchaserMtx.Unlock()
}

// MinFee returns the min ticket fee per KB to use when purchasing tickets.
func (t *TicketPurchaser) MinFee() cdrutil.Amount {
	t.purchaserMtx.Lock()
	minFee := t.minFee
	t.purchaserMtx.Unlock()
	return minFee
...
func (t *TicketPurchaser) SetMinFee(minFee int64) {
	t.purchaserMtx.Lock()
	t.minFee = cdrutil.Amount(minFee)
	t.purchaserMtx.Unlock()
}

// VotingAddress returns the address to send ticket outputs to.
func (t *TicketPurchaser) VotingAddress() cdrutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetVotingAddress(votingAddress cdrutil.Address) {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) PoolAddress() cdrutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetPoolAddress(poolAddress cdrutil.Address) {
	t.purchaserMtx.Lock()
...
func NewTicketPurchaser(cfg *Config,
	...
	return &TicketPurchaser{
		...
		account:           account,
		balanceToMaintain: cdrutil.Amount(cfg.BalanceToMaintainAbsolute),
		maxFee:            cdrutil.Amount(cfg.MaxFee),
		minFee:            cdrutil.Amount(cfg.MinFee),
		maxPerBlock:       cfg.MaxPerBlock,
		maxPriceAbsolute:  cdrutil.Amount(cfg.MaxPriceAbsolute),
...
type PurchaseStats struct {
	...
	PriceAverage  cdrutil.Amount
	PriceNext     cdrutil.Amount
	PriceCurrent  cdrutil.Amount
	Purchased     int
	LeftWindow    int
	Balance       cdrutil.Amount
	TicketPrice   cdrutil.Amount
}
...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if t.firstStart {
		...
		log.Debugf("Transaction relay fee: %v", cdrutil.Amount(t.cfg.TxFee))
	...
	if refreshStakeInfo {
		...
		for i := 1; i <= stakeInfoReqTries; i++ {
			curStakeInfo, err = t.wallet.StakeInfo(t.cdrChainSvr)
			if err != nil {
		...
		t.stakeVoteSubsidy = cdrutil.Amount(subsidy)
	...
	if avgPriceAmt < cdrutil.Amount(t.activeNet.MinimumStakeDiff) {
		avgPriceAmt = cdrutil.Amount(t.activeNet.MinimumStakeDiff)
	}
	...
	sDiffEsts, err := t.cdrChainSvr.EstimateStakeDiff(nil)
	if err == nil {
		ps.PriceNext, err = cdrutil.NewAmount(sDiffEsts.Expected)
		if err != nil {
			return ps, err
		}
	...
	var maxPriceAmt cdrutil.Amount
	...
	oneBlock := uint32(1)
	info, err := t.cdrChainSvr.TicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
	...
	if !t.cfg.NoSpreadTicketPurchases && toBuyForBlock > 0 {
		...
		log.Debugf("Your average purchase price for tickets in the pool is %.2f cdr", yourAvgTixPrice)
		log.Debugf("Available funds of %.2f cdr can buy %.2f tickets, %.2f tickets per block",
		...
		log.Debugf("Redeemed ticket value expected is %.2f cdr, buys %.2f tickets, %.2f%% more",
			redeemedFunds, tixToBuyWithRedeemedFunds, tixToBuyWithRedeemedFunds/tixCanBuy*100)
		log.Debugf("Stake reward expected is %.2f cdr, buys %.2f tickets, %.2f%% more",
	...
	// have enough moneys.
	notEnough := func(bal cdrutil.Amount, toBuy int, sd cdrutil.Amount) bool {
	...
	// Purchase tickets.
	poolFeesAmt, err := cdrutil.NewAmount(t.cfg.PoolFees)
	if err != nil {
		return ps, err
	}
