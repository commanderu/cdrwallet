...
GOVERSION=${1:-1.10}
REPO=cdrwallet
DOCKER_IMAGE_TAG=commanderu-golang-builder-$GOVERSION
...
docker pull commanderu/$DOCKER_IMAGE_TAG
if [ $? != 0 ]; then
        echo 'docker pull failed'
        exit 1
fi

docker run --rm -it -v $(pwd):/src commanderu/$DOCKER_IMAGE_TAG /bin/bash -c "\
  rsync -ra --filter=':- .gitignore'  \
  /src/ /go/src/github.com/commanderu/$REPO/ && \
  cd github.com/commanderu/$REPO/ && \
