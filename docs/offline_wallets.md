...
```
cdrwallet --create --createwatchingonly
```
...
granted that cdr and cdrwallet are installed and vendored dependencies 
are up to date with dep:

```
cd $GOPATH/src/github.com/cdrwallet/cmd/movefunds
go install
```
...
1. On the machine with the watching only wallet, call 'listunspent' and pipe 
    the output to unspent.json (cdrctl --wallet listunspent > unspent.json). 
	Next, run:
	```
	cdrctl --wallet accountaddressindex myAccountName 0
	cdrctl --wallet accountaddressindex myAccountName 1
	```
...
3. Open a terminal and change directory to where unspent.json is. Then, copy 
    config.json from $GOPATH/src/github.com/cdrwallet/cmd/movefunds to 
	this directory.
	```
	cp $GOPATH/src/github.com/cdrwallet/cmd/movefunds/config.json config.json
	```
...
6. Connect cdrwallet on the cold machine. Synchronize the addresses on this 
    wallet using the command and the responses you got at step 1:
	```
	cdrctl --wallet accountsyncaddressindex myAccountName 0 <response1>
	cdrctl --wallet accountsyncaddressindex myAccountName 1 <response2>
	```
...
8. Send the raw transaction on the hot wallet machine.
    ```
	cdrctl sendrawtransaction $(cat rawtx.txt)
    ```
