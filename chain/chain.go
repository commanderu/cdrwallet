...
// RPCClient represents a persistent client connection to a commanderu RPC server
// for information regarding the current best block chain.
type RPCClient struct {
	*cdrrpcclient.Client
	connConfig  *cdrrpcclient.ConnConfig // Work around unexported field
...
// operate on the same commanderu network as described by the passed chain
// parameters, the connection will be disconnected.
func NewRPCClient(chainParams *chaincfg.Params, connect, user, pass string, certs []byte,
	disableTLS bool) (*RPCClient, error) {

	client := &RPCClient{
		connConfig: &cdrrpcclient.ConnConfig{
			Host:                 connect,
			Endpoint:             "ws",
			User:                 user,
			Pass:                 pass,
	...
	ntfnCallbacks := &cdrrpcclient.NotificationHandlers{
	...
	rpcClient, err := cdrrpcclient.New(client.connConfig, ntfnCallbacks)
	if err != nil {
		return nil, err
	}
...
func (c *RPCClient) Start(ctx context.Context, retry bool) (err error) {
	...
	if err == nil {
		versionResult := versions["cdrjsonrpcapi"]
		serverAPI = semver{
...
// cdrrpcclient callbacks, which isn't very Go-like and doesn't allow
...
// commanderu RPC server.  This channel must be continually read or the process
...
out:
	for {
		...
		case <-pingChan:
			...
			// be due to cdr processing websocket requests synchronously (see
...
// POSTClient creates the equivalent HTTP POST cdrrpcclient.Client.
func (c *RPCClient) POSTClient() (*cdrrpcclient.Client, error) {
	configCopy := *c.connConfig
	configCopy.HTTPPostMode = true
	return cdrrpcclient.New(&configCopy, nil)
}
