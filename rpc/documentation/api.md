...
the management of the wallet and its connection to the commanderu network.  It has
...
dynamically start the cdr RPC client.  This RPC client is used for wallet
syncing and publishing transactions to the commanderu network.
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default cdr RPC port of the
  active commanderu network.
...
- `string public_key`: The public key encoded as a string in the commanderu encoding
  format.
...
the commanderu network.  If the transaction spends any of the wallet's unspent
...
notifications passed from the JSON RPC of cdr. Right now, wallet block processing
