...
//
//   https://github.com/commanderu/cdrwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/commanderu/cdrwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	"github.com/commanderu/cdrwallet/internal/cfgutil"
	h "github.com/commanderu/cdrwallet/internal/helpers"
	"github.com/commanderu/cdrwallet/internal/zero"
	"github.com/commanderu/cdrwallet/loader"
	"github.com/commanderu/cdrwallet/netparams"
	"github.com/commanderu/cdrwallet/rpc/legacyrpc"
	pb "github.com/commanderu/cdrwallet/rpc/walletrpc"
	"github.com/commanderu/cdrwallet/ticketbuyer"
	...
	"github.com/commanderu/cdrwallet/walletseed"
)
...
// cdrutil.DecodeAddress, which does not perform the network check.
func decodeAddress(a string, params *chaincfg.Params) (cdrutil.Address, error) {
	addr, err := cdrutil.DecodeAddress(a)
	if err != nil {
...
// as well as establishing a RPC connection to a cdr consensus server.
...
func (s *walletServer) NextAddress(ctx context.Context, req *pb.NextAddressRequest) (
	*pb.NextAddressResponse, error) {
	...
	pubKeyAddr, err := cdrutil.NewAddressSecpPubKey(pubKey.Serialize(), s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := cdrutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument,
...
func (s *walletServer) ImportScript(ctx context.Context,
	...
	p2sh, err := cdrutil.NewAddressScriptHash(req.Script, s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) BlockInfo(ctx context.Context, req *pb.BlockInfoRequest) (*pb.BlockInfoResponse, error) {
	...
	return &pb.BlockInfoResponse{
		...
		ApprovesParent:   header.VoteBits&cdrutil.BlockValid != 0,
	}, nil
}
...
func (s *walletServer) UnspentOutputs(req *pb.UnspentOutputsRequest, svr pb.WalletService_UnspentOutputsServer) error {
	...
	_, inputs, scripts, err := s.wallet.SelectInputs(cdrutil.Amount(req.TargetAmount), policy)
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	totalAmount, inputs, scripts, err := s.wallet.SelectInputs(cdrutil.Amount(req.TargetAmount), policy)
	// Do not return errors to caller when there was insufficient spendable
	// outputs available for the target amount.
	switch err.(type) {
	case nil, txauthor.InputSourceError:
	...
	if req.IncludeChangeScript && totalAmount > cdrutil.Amount(req.TargetAmount) {
...
func (s *walletServer) ConstructTransaction(ctx context.Context, req *pb.ConstructTransactionRequest) (
	*pb.ConstructTransactionResponse, error) {
	...
	if req.FeePerKb != 0 {
		feePerKb = cdrutil.Amount(req.FeePerKb)
	}
...
func (s *walletServer) PurchaseTickets(ctx context.Context,
	...
	// purchase request.
	spendLimit := cdrutil.Amount(req.SpendLimit)
	if spendLimit < 0 {
		return nil, status.Errorf(codes.InvalidArgument,
	...
	txFee := cdrutil.Amount(req.TxFee)
	ticketFee := s.wallet.TicketFeeIncrement()

	// Set the ticket fee if specified
	if req.TicketFee > 0 {
		ticketFee = cdrutil.Amount(req.TicketFee)
	}
...
func (s *walletServer) ValidateAddress(ctx context.Context, req *pb.ValidateAddressRequest) (*pb.ValidateAddressResponse, error) {
	...
	case udb.ManagedPubKeyAddress:
		...
		pubKeyAddr, err := cdrutil.NewAddressSecpPubKey(result.PubKey,
...
func marshalBlock(v *wallet.Block) *pb.BlockDetails {
	...
	return &pb.BlockDetails{
		...
		ApprovesParent: v.Header.VoteBits&cdrutil.BlockValid != 0,
		Transactions:   txs,
	}
}
...
func (s *loaderServer) StartConsensusRpc(ctx context.Context, req *pb.StartConsensusRpcRequest) (
	*pb.StartConsensusRpcResponse, error) {
	...
	if err != nil {
		if err == cdrrpcclient.ErrInvalidAuth {
			return nil, status.Errorf(codes.InvalidArgument,
...
func (s *messageVerificationServer) VerifyMessage(ctx context.Context, req *pb.VerifyMessageRequest) (
	*pb.VerifyMessageResponse, error) {
	...
	addr, err := cdrutil.DecodeAddress(req.Address)
	if err != nil {
		return nil, translateError(err)
	}
