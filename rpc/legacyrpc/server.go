...
// functional commanderu wallet RPC server.  This can be called to enable RPC
...
// fully functional commanderu wallet RPC server.
...
// method.  This may be a request that is handled directly by cdrwallet, or
// a chain server request that is handled by passing the request down to cdr.
//
...
func (s *Server) handlerClosure(ctx context.Context, request *cdrjson.Request) lazyHandler {
...
// but this is only implemented here to work around an oddity with cdrjson,
...
func (s *Server) invalidAuth(req *cdrjson.Request) bool {
	cmd, err := cdrjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*cdrjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := cdrjson.MarshalResponse(req.Jsonrpc, req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := cdrjson.MarshalResponse(req.Jsonrpc, req.ID, nil, cdrjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response to client %s: %v",
				r.RemoteAddr, err)
	...
	case "stop":
		...
		stop = true
		res = "cdrwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(ctx, &req)()
	}

	// Marshal and send.
	mresp, err := cdrjson.MarshalResponse(req.Jsonrpc, req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response to client %s: %v",
			r.RemoteAddr, err)
