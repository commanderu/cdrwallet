...
var (
	walletDataDirectory = cdrutil.AppDataDir("cdrwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet               bool                `long:"testnet" description:"Use the test commanderu network"`
	SimNet                bool                `long:"simnet" description:"Use the simulation commanderu network"`
...
func init() {
	...
	if opts.TestNet && opts.SimNet {
		fatalf("Multiple commanderu networks may not be used simultaneously")
	}
...
func makeInputSource(outputs []cdrjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue cdrutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := cdrutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(cdrutil.Amount) (cdrutil.Amount, []*wire.TxIn, [][]byte, error) {
...
// by this function.
func makeDestinationScriptSourceToAccount(rpcClient *cdrrpcclient.Client, accountName string) txauthor.ChangeSource {
...
func makeDestinationScriptSourceToAddress(rpcClient *cdrrpcclient.Client, address string) txauthor.ChangeSource {
	return func() ([]byte, uint16, error) {
...
func sweep() error {
	...
	rpcClient, err := cdrrpcclient.New(&cdrrpcclient.ConnConfig{
	...
	sourceOutputs := make(map[string][]cdrjson.ListUnspentResult)
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := cdrutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount cdrutil.Amount) bool {
	return amount >= 0 && amount <= cdrutil.MaxAmount
}

func parseOutPoint(input *cdrjson.ListUnspentResult) (wire.OutPoint, error) {
