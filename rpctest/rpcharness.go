...
import (
	...
	rpc "github.com/commanderu/cdr/rpcclient"
)
...
// Harness fully encapsulates an active cdr process, along with an embedded
// cdrwallet to provide a unified platform for creating RPC-driven integration
// tests involving cdr. The active cdr node will typically be run in simnet
...
// spends. The active cdr process is fully managed by Harness, which handles
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// Start the cdr node itself. This spawns a new process which will be
	// managed
	...
	// Start cdrwallet. This spawns a new process which will be managed
	...
	// Get a new address from the wallet to be set with cdr's --miningaddr
	time.Sleep(5 * time.Second)
	var miningAddr cdrutil.Address
	for i := 0; i < 100; i++ {
...
// cdr process belonging to this Harness instance. If the initial connection
