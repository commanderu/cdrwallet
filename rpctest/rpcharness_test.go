...
import (
	...
	"github.com/commanderu/cdrwallet/rpc/legacyrpc"
)
...
func TestMain(m *testing.M) {
	...
	ntfnHandlersNode := cdrrpcclient.NotificationHandlers{
...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		// Decode address
		_, err = cdrutil.DecodeAddress(addrStr)
		if err != nil {
			t.Fatalf("Unable to decode address %s: %v", addr.String(), err)
		}

		// Try to validate an address that is not owned by wallet
		otherAddress, err := cdrutil.DecodeAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
	...
	DevAddr, err := cdrutil.DecodeAddress(devSubAddrStr)
	if err != nil {
...
func testWalletPassphrase(r *Harness, t *testing.T) {
	...
	if err != nil && err.(*cdrjson.RPCError).Code !=
		cdrjson.ErrRPCWalletPassphraseIncorrect {
		// cdrjson.ErrWalletPassphraseIncorrect.Code
	...
	// cdrjson.ErrRPCWalletUnlockNeeded
	...
	if err != nil && err.(*cdrjson.RPCError).Code !=
		cdrjson.ErrRPCWalletAlreadyUnlocked {
		t.Fatalf("WalletPassphrase failed: %v", err)
	}
...
func testGetBalance(r *Harness, t *testing.T) {
	...
	preAccountBalances := make(map[string]cdrjson.GetAccountBalanceResult)
	...
	sendAmount := cdrutil.Amount(700000000)
	...
	postAccountBalances := make(map[string]cdrjson.GetAccountBalanceResult)
...
func testListUnspent(r *Harness, t *testing.T) {
	...
	amountToSend := cdrutil.Amount(700000000)
	...
	// Get *cdrutil.Tx of send to check the inputs
	...
	for _, txIn := range rawTx.MsgTx().TxIn {
		...
		txInIDs[prevOut.String()] = cdrutil.Amount(txIn.ValueIn).ToCoin()
	}
	...
	for txinID, amt := range txInIDs {
		if _, ok := utxosBeforeSend[txinID]; !ok {
			t.Fatalf("Failed to find txid %v (%v cdr) in list of UTXOs",
				txinID, amt)
		}
	}
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	amountToSend := cdrutil.Amount(1000000)
	...
	fee := cdrutil.Amount(totalSpent - totalSent)
...
func testSendMany(r *Harness, t *testing.T) {
	...
	amountsToSend := []cdrutil.Amount{700000000, 1400000000}
	addresses := []cdrutil.Address{}
	...
	addressAmounts := make(map[cdrutil.Address]cdrutil.Amount)
	totalAmountToSend := cdrutil.Amount(0)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	if *txList1[0].TxType != cdrjson.LTTTRegular {
	...
	voutAmt := cdrutil.Amount(txOut.Value).ToCoin()
	...
	sendAmount := cdrutil.Amount(240000000)
	...
	var sendResult, recvResult cdrjson.ListTransactionsResult
	...
	rxtxResults := map[string]cdrjson.ListTransactionsResult{
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	// cdrrpcclient does not have a getwalletfee or any direct method, so we
	...
	origTxFee, err := cdrutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	// Increase fee by 50%
	newTxFeeCoin := walletInfo.TxFee * 1.5
	newTxFee, err := cdrutil.NewAmount(newTxFeeCoin)
	if err != nil {
	...
	newTxFeeActual, err := cdrutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	...
	err = wcl.SetTxFee(cdrutil.Amount(-1))
	if err == nil {
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	// cdrrpcclient does not have a getticketee or any direct method, so we
	...
	origTicketFee, err := cdrutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	newTicketFee, err := cdrutil.NewAmount(newTicketFeeCoin)
	if err != nil {
	...
	nominalTicketFee = walletInfo.TicketFee
	newTicketFeeActual, err := cdrutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	priceLimit, err := cdrutil.NewAmount(2 * mustGetStakeDiffNext(r, t))
	if err != nil {
	...
	err = wcl.SetTicketFee(cdrutil.Amount(-1))
	if err == nil {
...
func testPurchaseTickets(r *Harness, t *testing.T) {
	...
	origTicketFee, err := cdrutil.NewAmount(walletInfo.TicketFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
	newTicketFee, err := cdrutil.NewAmount(walletInfo.TicketFee * 1.5)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
...
func mustGetStakeInfo(wcl *cdrrpcclient.Client, t *testing.T) *cdrjson.GetStakeInfoResult {
	stakeinfo, err := wcl.GetStakeInfo()
	if err != nil {
		t.Fatal("GetStakeInfo failed: ", err)
	}
...
func newBlockAt(currentHeight uint32, r *Harness,
	t *testing.T) (uint32, *cdrutil.Block, []*chainhash.Hash) {
...
func newBlockAtQuick(currentHeight uint32, r *Harness,
	...
	return block.Header.Height, cdrutil.NewBlock(block), blockHashes
}

func getBestBlock(r *Harness, t *testing.T) (uint32, *cdrutil.Block, *chainhash.Hash) {
	...
	return curBlockHeight, cdrutil.NewBlock(bestBlock), bestBlockHash
}
...
func includesTx(txHash *chainhash.Hash, block *cdrutil.Block) bool {
	if len(block.Transactions()) <= 1 {
		return false
	}
...
// includesTx checks if a block contains a transaction hash
func includesStakeTx(txHash *chainhash.Hash, block *cdrutil.Block) bool {
...
// spent minus sent.
func getWireMsgTxFee(tx *cdrutil.Tx) cdrutil.Amount {
	var totalSpent int64
	...
	return cdrutil.Amount(totalSpent - totalSent)
}

// getOutPointString uses OutPoint.String() to combine the tx hash with vout
// index from a ListUnspentResult.
func getOutPointString(utxo *cdrjson.ListUnspentResult) (string, error) {
